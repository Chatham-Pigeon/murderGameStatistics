...
with open(fr'../data/{time}.{log_name}-data.txt', 'r') as file:
    ...
    for line in file:
        ...
        if not data.playercount in role_wins_game_length_per_map:
            role_wins_game_length_per_map[data.playercount] = {"Traitors": [], "Citizens": [], "Fiends": []}
        if data.fiend == 'false':
            role_wins_proper[data.winner] += 1
            role_wins_game_length[data.winner].append(data.length)
            role_wins_game_length_per_map[data.playercount][data.winner].append(data.length)
        else:
            role_wins_proper['Fiends'] += 1
            role_wins_game_length["Fiends"].append(data.length)
            role_wins_game_length_per_map[data.playercount]["Fiends"].append(data.length)

        if not data.playercount in map_wins:
            map_wins[data.playercount] = {"Traitors": 0, "Citizens": 0, "Fiends": 0}
        if data.fiend == 'false':
...
print(f"Playercounts ranked by their Euclidian Distance Deviation from a 40/40/20% win rate, lower is better. (avg is {avg_deviation}]")
